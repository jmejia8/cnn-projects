...
train_set_idx = randperm(50000)[1:500]

X = trainimgs(CIFAR10)
imgs = [getarray(X[i].img) for i in train_set_idx]
labels = onehotbatch([X[i].ground_truth.class for i in train_set_idx],1:10)
train = gpu.([(cat(imgs[i]..., dims = 4), labels[:,i]) for i in partition(train_set_idx, 100)])
valset = collect(49001:50000)
valset = valset[randperm(length(valset))[1:100]]
